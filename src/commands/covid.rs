...
pub async fn covid(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
	...
	let response = reqwest::get("https://api.corona-zahlen.org/germany/").await?.json::<Corona>().await?;
	...
	match value.as_str() {
		...
		_ => message.push_str(&format!("Invalid Request: `{}`! Try again with `c`, `d`, `i`, `h`, `r` or `s`:", value)),
	};
